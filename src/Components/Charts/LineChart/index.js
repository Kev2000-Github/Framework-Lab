...
export const LineChart = ({years=[] , dataReal=[], dataProjection=[]}) => {
    ...
    return (
        <div className='lineChart'>
            ...
            <div className='lineSubHeader'>
                <h1>${numberFormatter(totalSales)}</h1>
                <p className={`diffPercent ${isDecrease ? 'decrease' : 'increase'}`}>
                    {isDecrease ? '' : '+'}{diffPercent.toFixed(2)}%
                </p>
            </div>
            <div className='avgSales'>
                <p>${numberFormatter(totalSales/dataReal.length | 0)} avg sales</p>
            </div>
